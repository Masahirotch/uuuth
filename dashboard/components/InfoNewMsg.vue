...
export default {
    ...
    data() {
        return {
            ...
            to            : ['U46178fcc5f1bf5c8508fe3a3876475b3','U775973dbc947217bf95755ed3adf15ea','U953f712fe36fed4303e9543676dd025d','U5486742be994d497c7b116b855a991b8','U908ed239c337ccc303960bd1eb93ff2e'],
            sendTo        : [],
            fullUserNum   : 0,
    ...
    methods:{
        ...
        ******************************************/
        testSending : async function(){
            ...
            params.append('action'     , 'multicast');
        ...
        ******************************************************************/
        nowSending : async function(){
            ...
            params.append('token'      , this.apiToken );
            params.append('action'     , 'multicast');
            params.append('to'         , JSON.stringify( send ) );
            params.append('msgArray'   , JSON.stringify( this.messages ) );
        ...
        ******************************************************************/
        sendMessageUpdate : async function(){
            ...
            var i_date = ( this.inputDate != '' )? this.$dayjs(this.inputDate).format('YYYY-MM-DD'):'';

            const params = new URLSearchParams();
            params.append( 'token' , process.env.INFO_TOKEN )
            params.append('action'        , 'updateSendDate');
            params.append('msgArray'      , JSON.stringify( this.messages ) );
        ...
        ******************************************************************/
        saveDraft : async function(){
            ...
            .then( function (result){
                ...
                this.newMessageId = JSON.parse( JSON.stringify( result.data.ID ) )
