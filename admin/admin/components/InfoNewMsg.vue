...
export default {
    ...
    data() {
        return {
            ...
            to            : process.env.INFO_TEST_MSG_TO,
            sendTo        : [],
            fullUserNum   : 0,
    ...
    watch : {
        ...
        console.dir(value)
          ...
          let userConditions = new Object()
          userConditions.flg_active = 1
          userConditions.$or = []
          let flgName = null
          value.forEach(flgNum => {
            flgName = 'flg_' + flgNum
            userConditions.$or.push({[flgName]: 1})
          })

          axios.post( process.env.API_URL + 'getUsers' , {
            token: process.env.INFO_TOKEN,
            where_flg: userConditions,
          } )
            .then( function(result){

              this.sendTo = JSON.parse( JSON.stringify( result.data.foundUsers ) )
              console.dir(this.sendTo)
    ...
    methods:{
        ...
        getAllUserNum : function(){
            ...
            axios.post( process.env.API_URL + 'getUsers' , {
              ...
              .then( function(result){
                ...
                this.fullUserNum = result.data.foundUsers.length
        ...
        ******************************************************************/
        ...
        removeMessage : function(index){
            ...
            this.$confirm( 'このメッセージを削除しますか？','確認', {
        ...
        *********************************************************/
        imageSetting : async function(targetIndex){
            ...
            // 画像のみをCDNにアップロード ////////////////////////////////////
            await axios.post( process.env.API_URL + 'uploadFileCDN' , {
              token: this.apiToken,
              imageBase64: this.fileUrl,
            })
            .then( function(result){

                this.messages[targetIndex].originalContentUrl = result.data.files.orgn
                this.messages[targetIndex].previewImageUrl    = result.data.files.thmb
        ...
        movieSetting : async function(targetIndex){
            ...
            .then( function(result){
              ...
              if( result.data == 'error'){
                ...
                this.messages[targetIndex].originalContentUrl = JSON.parse( JSON.stringify( result.data.files.vdeo ))
                this.messages[targetIndex].previewImageUrl    = JSON.parse( JSON.stringify( result.data.files.thmb ))
        ...
        ******************************************/
        testSending : async function(){
            ...
            let msgArray = this.getMsgArray()

            await axios.post( process.env.PUSH_API + 'multicast' , {
              token: this.apiToken,
              config: {
            ...
            .then( function (result){
                ...
                this.popAlert('テストメッセージを送信しました。','Sending Test Message.')
        ...
        ******************************************************************/
        nowSending : async function(){
            ...
            if( this.sendTo == null ){
                ...
                this.popAlert('配信先が設定されていません。','No User.')
                return
            ...
            const send = this.sendTo.map( x => x = x.userId );
        ...
        /* メッセージ送信日時のアップデート
        ******************************************************************/
        sendMessageUpdate : async function(){

            var i_date = ( this.inputDate != '' )? this.$dayjs(this.inputDate).format('YYYY-MM-DD'):null;

            let set_message = null
            let post_string = null
            console.log(this.newMessageId)
            if (this.newMessageId) {
              post_string = 'updateMessage'
              set_message = {
                createDate: '',
                sendDate: this.sendDate,
                inputDate: (i_date ? i_date : null), 
            ...
            await axios.post( process.env.API_URL + post_string , {
              token: process.env.INFO_TOKEN,
            ...
            .then( function (result){
                ...
                this.$alert('メッセージを送信しました。','Sending Message.', {
                    confirmButtonText: 'OK',
                    callback: action => {
        ...
        ******************************************************************/
        saveDraft : async function(){
            ...
            params.append('destinationTo' , JSON.stringify( this.destinationTo ) );
            params.append('sendDate'      , this.sendDate );
            params.append('inputDate'     , i_date );
            params.append('inputTime'     , this.inputTime );
            params.append('newMessageId'  , this.newMessageId );
            ...
            .then( function (result){
                ...
                this.newMessageId = JSON.parse( JSON.stringify( result.data.message.ID ) )
                this.popAlert('下書きを保存しました。','Save Draft Data.')
